...
{
    public Transform target;               // Referencia al jugador (torso)
    public Transform head;                 // Referencia a la cabeza del jugador
    public Transform flashlight;           // Referencia a la linterna
    ...
    private void LateUpdate()
    {
        ...
        // Usar el target (cuerpo) para la posición de la cámara
        ...
        // Rotar la linterna para que apunte en la misma dirección que la cámara
        if (flashlight != null)
        {
            flashlight.rotation = rotation;  // Asegurarse de que la linterna rota igual que la cámara
        }
    }
}